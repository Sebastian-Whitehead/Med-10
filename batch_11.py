...
from utils.utilities import get_root_dir, update_batch_id
...
def evaluate_batch_11(model, batch_path, log=False, std=False, set=None):
    ...
    time_per_image = time_total / len(image_paths)

    if std:
        individual_map_values_50 = []
        individual_map_values_95 = []
        recall_values = []
        precision_values = []
    ...
    if log:
        log_results_to_json(
            update_batch_id(), batch_path, "11", set, eval_metric.map50, eval_metric.map50_95,
            pre.precision_at_50, rec.recall_at_50, eval_metric.matched_classes, eval_metric.ap_per_class,
            std_dev_50, std_dev_95, std_recall, std_precision, time_total, time_per_image, len(image_paths)
        )
